...
class MainActivity : AppCompatActivity() {
    ...
    private val url = "https://jsonplaceholder.typicode.com/posts" //equipo Luis Garcia
    ...
    private fun downloadTask() {
        ...
        val stringRequest = StringRequest(
            ...
            { response ->
                try {
                    val jsonArray = JSONArray(response) //equipo Luis Garcia

                    // ---- CÓDIGO MEJORADO ----
                    // Bucle para recorrer cada objeto en el array
                    for (i in 0 until jsonArray.length()) {
                        val jsonObject = jsonArray.getJSONObject(i)
                        val id = jsonObject.getInt("id") //equipo Luis Garcia
